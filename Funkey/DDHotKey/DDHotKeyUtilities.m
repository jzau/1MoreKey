...
extern NSString *DDStringFromKeyCode(unsigned short keyCode, NSUInteger modifiers) {
    return [NSString stringWithFormat:@"%@%@", DDStringFromModifier(modifiers), DDStringFromKeyCodeOnly(keyCode)];
}

NSString *DDStringFromModifier(NSUInteger modifiers) {
    NSMutableString *final = [NSMutableString stringWithString:@""];
    NSDictionary *characterMap = _DDKeyCodeToCharacterMap();
    
    if (modifiers & NSEventModifierFlagControl) {
        if (modifiers & NX_DEVICELCTLKEYMASK) {
            [final appendString:@"Left"];
        }
        if (modifiers & NX_DEVICERCTLKEYMASK) {
            [final appendString:@"Right"];
        }
    ...
    if (modifiers & NSEventModifierFlagShift) {
        if (modifiers & NX_DEVICELSHIFTKEYMASK) {
            [final appendString:@"Left"];
        }
        if (modifiers & NX_DEVICERSHIFTKEYMASK) {
            [final appendString:@"Right"];
        }
...
//        return final;
//    }
NSString * DDStringFromKeyCodeOnly(unsigned short keyCode) {
    NSMutableString *final = [NSMutableString stringWithString:@""];
    NSDictionary *characterMap = _DDKeyCodeToCharacterMap();
...
NSUInteger DDCarbonCocoaModifiersFromModifierFlags(UInt32 flags) {
    NSUInteger newFlags = 0;
    if ((flags & controlKey) > 0) { newFlags |= NSEventModifierFlagControl; }
    if ((flags & cmdKey) > 0) { newFlags |= NSEventModifierFlagCommand; }
    if ((flags & shiftKey) > 0) { newFlags |= NSEventModifierFlagShift; }
    if ((flags & optionKey) > 0) { newFlags |= NSEventModifierFlagOption; }
    if ((flags & alphaLock) > 0) { newFlags |= NSEventModifierFlagCapsLock; }
    return newFlags;
}
