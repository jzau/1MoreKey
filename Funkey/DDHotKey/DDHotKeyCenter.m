...
OSStatus dd_hotKeyHandler(EventHandlerCallRef nextHandler, EventRef theEvent, void *userData) {
    @autoreleasepool {
        ...
        NSEvent *keyUpEvent = [NSEvent keyEventWithType:NSEventTypeKeyUp
                                                 location:[event locationInWindow]
                                            modifierFlags:[event modifierFlags]
                                                timestamp:[event timestamp]
                                             windowNumber:-1
                              ...
                              charactersIgnoringModifiers:@""
                                                isARepeat:NO
                                                  keyCode:[matchingHotKey keyCode]];
        if (event.modifierFlags == matchingHotKey.modifierFlags) {
            [matchingHotKey invokeWithEvent:keyDownEvent];
        }else {
            [[DDHotKeyCenter sharedHotKeyCenter] unregisterHotKey:matchingHotKey];
            CGEventPost(kCGSessionEventTap, keyDownEvent.CGEvent);
            CGEventPost(kCGSessionEventTap, keyUpEvent.CGEvent);
            [[DDHotKeyCenter sharedHotKeyCenter] registerHotKey:matchingHotKey];
        }
    }
